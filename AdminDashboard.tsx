...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
  ...
  const handleManualSync = async () => {
    if (!sheetUrl) return alert("Configura primero la URL del Google Script en Ajustes");
    if (!confirm("Esto sobrescribirá la hoja de cálculo con los datos actuales de esta App. ¿Continuar?")) return;
    
    setIsSyncing(true);
    await onSyncToCloud();
    setIsSyncing(false);
    alert("Sincronización completada. Verifica tu Google Sheet.");
  };
        ...
        <div className="space-y-6">
          <div className="flex gap-4 flex-col sm:flex-row">
            ...
            <button 
              ...
              className={`px-8 py-5 rounded-3xl font-black uppercase tracking-widest shadow-xl transition-all flex items-center justify-center gap-3 active:scale-95 border ${!sheetUrl ? 'bg-slate-100 text-slate-400 border-slate-200 cursor-not-allowed' : 'bg-emerald-500 text-white border-emerald-500 hover:bg-emerald-600'}`}
            >
              {isSyncing ? (
              ...
              {isSyncing ? 'Subiendo...' : 'Subir a Nube'}
            </button>
          </div>
        ...
        <div className="max-w-md mx-auto space-y-8 bg-white p-10 rounded-[3rem] border border-slate-100 shadow-xl animate-in slide-in-from-bottom duration-500">
          ...
          <div className="space-y-6">
            ...
            <button 
              ...
              className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black uppercase tracking-widest shadow-lg hover:bg-slate-800 transition-all"
            >
              Guardar Cambios
            </button>
